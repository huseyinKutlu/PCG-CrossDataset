...
if __name__ == "__main__":
    ...
    # Resmi mamba_ssm yalnizca CUDA'da calisir. CUDA varsa GPU'da test et.
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Test device: {device}")

    # Small tensors keep the smoke test fast. Use real lengths during training.
    raw = torch.randn(1, 1, 512, device=device)
    mel = torch.randn(1, 1, 64, 32, device=device)
    cwt = torch.randn(1, 1, 64, 32, device=device)
    site = torch.tensor([0], device=device)

    # CPU'da resmi Mamba calismaz -> CPU ise fallback'e zorla
    prefer_mamba = (device.type == "cuda")
    model = PCGMambaConformer(n_classes=2, branch_dim=32, pretrained=False,
                              n_sites=4, prefer_official_mamba=prefer_mamba).to(device)
    model.eval()
    with torch.no_grad():
    ...
    patient_probs = aggregate_patient_probabilities(logits.cpu(), patient_ids=["p1"], method="mean_prob")
