...
"""
22_mmd_da.py
PCG Q1 Manuscript — MMD (Maximum Mean Discrepancy) domain adaptation.

AMAC: A-distance=1.70 (ozellikler ayrisik). MMD (sadece kovaryans=2.moment)
yetersiz kaldi. MMD, multi-bandwidth RBF kernel ile TUM momentleri hizalar —
MMD'in yakalayamadigi dagilim farkini yakalayabilir.

YONTEM (Sun & Saenko 2016):
  L = L_cls(CirCor, etiketli) + lambda * L_mmd(CirCor_feat, CinC_feat)
...
Kullanim:
  python 19_mmd_da.py --model hybrid_kan --lambda_mmd 1.0 --epochs 30
  python 19_mmd_da.py --model cnn_bilstm --lambda_mmd 1.0 --epochs 30
"""
...
CKPT_DIR = PROJECT_ROOT / "checkpoints" / "mmd"; CKPT_DIR.mkdir(parents=True, exist_ok=True)
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
N_FOLDS = 5
CIRCOR_TO_BINARY = {0: 1, 1: 1, 2: 0}


# ---- MMD kaybi (multi-bandwidth RBF) ----
def mmd_loss(source, target, sigmas=(1, 2, 4, 8, 16)):
    """source, target: (n,d). Multi-bandwidth RBF MMD^2.
    MMD'dan farki: tum momentleri yakalar (sadece 2. moment degil)."""
    def pdist2(A, B):
        a2 = (A * A).sum(1, keepdim=True)
        b2 = (B * B).sum(1, keepdim=True).t()
        return (a2 + b2 - 2 * A @ B.t()).clamp(min=0)
    dxx = pdist2(source, source)
    dyy = pdist2(target, target)
    dxy = pdist2(source, target)
    Kxx = Kyy = Kxy = 0
    for s in sigmas:
        g = 1.0 / (2 * s * s)
        Kxx = Kxx + torch.exp(-g * dxx)
        Kyy = Kyy + torch.exp(-g * dyy)
        Kxy = Kxy + torch.exp(-g * dxy)
    m = source.shape[0]; n = target.shape[0]
    return Kxx.mean() + Kyy.mean() - 2 * Kxy.mean()


# ---- ozellik cikaran sarmalayici (head oncesi temsil) ----
def extract_features(model, raw, mel, mode):
    """Modelin head-oncesi fuzyon ozelligini dondur (MMD bunun uzerinde calisir)."""
    if mode == "dual":
...
def main():
    ...
    ap.add_argument("--lambda_mmd", type=float, default=1.0)
    ...
    log("="*70)
    log(f"DEEP MMD domain adaptation | model={args.model} | lambda={args.lambda_mmd}")
    ...
    for fold in range(N_FOLDS):
        ...
        ckpt = CKPT_DIR / f"{args.model}_mmd_fold{fold}.pth"

        for ep in range(1, args.epochs + 1):
            model.train()
            cinc_iter = iter(cinc_loader)
            tot_cls, tot_mmd, n = 0.0, 0.0, 0
            for b in src_loader:
                ...
                loss_mmd = mmd_loss(feat_s, feat_t) if feat_s is not None else torch.tensor(0.0, device=DEVICE)
                loss = loss_cls + args.lambda_mmd * loss_mmd
                if not torch.isfinite(loss): continue
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip)
                opt.step()
                tot_cls += loss_cls.item() * len(y)
                tot_mmd += float(loss_mmd.detach()) * len(y); n += len(y)
            sched.step()
        ...
        log(f"  fold {fold}: ic AUROC={in_au:.4f} | dis AUROC={ex_au:.4f} "
            f"(mmd={tot_mmd/max(n,1):.6f}, λ·mmd={args.lambda_mmd*tot_mmd/max(n,1):.5f}) "
            f"({time.time()-t0:.0f}s)")
